...
The PLC operation is a full state snapshot. Every regular `plc_operation` includes the required `alsoKnownAs` field as an array, including when it is empty. An implementation constructing a normal update starts from the complete validated current state; a recovery operation starts from the complete validated state at its selected pre-fork `prev`. It changes only entries explicitly authorized for that operation, because omitting an existing entry removes it from the resulting state. In particular, every Hail-constructed operation must preserve every non-Hail `alsoKnownAs` value from that baseline state in its existing order unless the DID controller explicitly authorizes its removal, replacement, or reordering as part of the exact full-state operation being signed. A newly created Hail identity uses an empty `alsoKnownAs` array, and no update adds a Hail address there. The encoded operation must remain within PLC's 7500-byte DAG-CBOR limit, the state must contain from one through five rotation keys, and the state must remain within PLC's limit of ten verification methods. Hail requires two of those methods. PLC DAG-CBOR is an external DID-method encoding and is distinct from Hail's deterministic CBOR profile.
