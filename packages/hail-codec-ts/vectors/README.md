# Hail v0 Conformance Vectors

`v0.json` is the checked-in, language-neutral conformance manifest for the Hail v0 encoding profile. It contains:
...
Package consumers can access the manifest through the exported `@hailproto/codec/vectors/v0.json` subpath. JSON module loading syntax remains runtime-specific; consumers may also read the file as package data.
...
Conformance tests consume `v0.json` directly. They do not invoke the generator. This makes the checked-in bytes the expected values and detects accidental encoding or signing changes.
